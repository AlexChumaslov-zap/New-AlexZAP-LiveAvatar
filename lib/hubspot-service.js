...
export function formatPayload(conversation, reports) {
  ...
  contactProps.message = lines.join("\n");

  return { contactProps };
}

/**
 * Create or upsert a HubSpot Contact from the formatted payload.
 * Uses batch upsert (deduplication by email) when an email is present;
 * falls back to a plain create (no deduplication) otherwise.
 * Returns { contactId }.
 */
