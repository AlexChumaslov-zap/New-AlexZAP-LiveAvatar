...
Return a single JSON object. The first three sections — QualificationAssessment, RecommendedNextActions, and JunkAssessment — should always be produced. The rest are optional: omit a section entirely when the conversation provides no relevant evidence. Never fabricate facts.
...
- JunkAssessment: { IsJunk: boolean, Reason: string }
    IsJunk is true when the conversation is entirely off-topic — the visitor exclusively discussed subjects unrelated to automation, AI, software testing, business operations, or process improvement (e.g., weather, personal advice, cooking, entertainment, sports).
    IsJunk is false for any conversation that contains even one relevant exchange about automation, testing, AI, business challenges, or the company's services.
    Reason: one sentence (required even when IsJunk is false — e.g., "Conversation is on-topic: visitor discussed test automation needs.").
