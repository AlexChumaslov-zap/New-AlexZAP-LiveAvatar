// Land 3d: send a conversation transcript via SMTP.
//
// Security notes:
// - All visitor-supplied content (transcript text, names, company) is HTML-escaped
//   before inlining into the email body. AlexZAP's equivalent did not do this and
//   was vulnerable to inbox XSS.
// - Rate-limited to 5 sends per IP per hour (in-memory; bypassed in `netlify dev`).
// - SMTP credentials live only in Netlify env vars; never hard-coded.
...
export const handler = async (event) => {
  ...
  if (!Array.isArray(transcript) || transcript.length === 0) {
    return {
      statusCode: 400,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ error: "empty_transcript" }),
    };
  }
  if (transcript.length > MAX_TRANSCRIPT_LENGTH) {
    return {
      statusCode: 400,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ error: "transcript_too_long" }),
    };
  }

  // Sanitize transcript: keep only role + text, cap text length, drop empties.
  ...
  if (!SMTP_HOST || !SMTP_USER || !SMTP_PASSWORD || !SMTP_FROM_EMAIL) {
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ error: "missing_smtp_env" }),
    };
  }
